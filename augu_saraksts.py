...
def plant_card(parent):
    ...
    tk.Label(card, text=latest_plant["desc"],
             bg=CARD_BG, wraplength=760, justify="left").pack(anchor="w")
...
def show_login():
    ...
    tk.Button(root, text="Reģistrēties",
              command=lambda: msg.config(
                  text="✅ Reģistrēts" if register(u.get(),p.get()) else "⚠️ Jau eksistē"
              )).pack()
...
def show_main():
    ...
    entry=tk.Entry(search,width=30); entry.pack(side="left",padx=5)
    ...
    result=tk.Text(root,width=90,height=26,bg=BG_MAIN,borderwidth=0)
    result.pack()
...
def identify_worker(path):
    ...
    best=max(res.result.classification.suggestions,
...
def save_current():
    ...
    messagebox.showinfo("Saglabāts","Augs pievienots 🌱")
